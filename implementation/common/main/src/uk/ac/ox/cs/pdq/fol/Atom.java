...
public class Atom extends Formula {
    ...
    public String toStringWithColors(Map<Term, Integer> termOccNumb) {
        if (this.toString == null) {
            StringBuilder builder = new StringBuilder();
            builder.append(this.predicate.getName());
            if (this.terms.length > 0) {
                ...
                for (int index = 0; index < this.terms.length; index++) {
                    ...
                    if (this.terms[index].toString().equals("*"))
                        builder.append("<font color=red>").append(this.terms[index]).append("</font>").toString();
                    else if (termOccNumb.get(this.terms[index]) > 1)
                        builder.append("<font color=orange>").append(this.terms[index]).append("</font>").toString();
                    else
                        builder.append(this.terms[index]).toString();
